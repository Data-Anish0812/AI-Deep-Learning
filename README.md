...
**Climate Time Series Forecasting**  focuses on forecasting average daily temperatures in Delhi using Facebook Prophet, a robust and interpretable model designed for time series analysis. The dataset contains historical climate records, and the model was trained without a separate test set, instead evaluated using rolling cross-validation. Key metrics such as MAE, RMSE, and MAPE were used to assess the model's accuracy over multiple forecast horizons. The results show reliable short-term predictions, with performance gradually decreasing over longer horizons — a typical behavior in time series forecasting. This project demonstrates how to apply Prophet for practical climate trend analysis and forecasting.
